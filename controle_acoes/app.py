...
def _gex_walls(series, spot, faixa=0.30):
    ...
    negativo). Cada item em `series` precisa da chave 'grupo' com um dos
    valores 'atual', 'proximo' ou 'demais'.

    Devolve tanto o líquido por grupo (call - put, para o gráfico empilhado
    principal) quanto os 6 componentes brutos — call/put × os 3 grupos,
    ...
    for o in series:
        ...
        d = ag.setdefault(round(o['K'], 2), dict(
            ...
            **{f'call_{gr}': 0.0 for gr in grupos},
            **{f'put_{gr}': 0.0 for gr in grupos},
        ))
        chave = o.get('grupo') if o.get('grupo') in grupos else 'demais'
    ...
    out = []
    for v in ag.values():
        item = {'strike': v['strike']}
        for gr in grupos:
            item[gr] = round(v[gr], 2)
            item[f'call_{gr}'] = round(v[f'call_{gr}'], 2)
            item[f'put_{gr}'] = round(v[f'put_{gr}'], 2)
        out.append(item)
...
def api_market_gamma(ticker):
    ...
    if exp == 'all':
        ...
        # (já vencidos) não entram em nenhum grupo.
        depois = [v for v in todos_vencs if v > exp]
        if depois:
            proximo_venc = depois[0]
        outros = depois
        vencs_walls = outros if modo_walls == 'todos' else outros[:6]
...
def api_trader_futuros(ativo):
    ...
    if spot and spot > 0 and series:
        ...
        todos_vencs = ((ex_data or {}).get('expirations') or []) if not ex_err else []
        # Só o que vem depois de 'exp': vencimentos anteriores (já vencidos
        # na prática) não entram em nenhum grupo dos Walls.
        depois = [v for v in todos_vencs if v > exp]
        proximo_venc = depois[0] if depois else None
        vencs_extra = depois if modo_walls == 'todos' else depois[:6]
        ...
        if vencs_extra:
            from concurrent.futures import ThreadPoolExecutor
            with ThreadPoolExecutor(max_workers=8) as pool:
                resultados_extra = list(pool.map(_puxa_venc, vencs_extra))
            for x, parte in zip(vencs_extra, resultados_extra):
                if not parte:
                    continue
                grupo = 'proximo' if x == proximo_venc else 'demais'
                series_walls.extend(dict(o, grupo=grupo) for o in parte)
...
@app.route('/api/fyt-trava-baixa/<ticker>')
@login_required
def api_fyt_trava_baixa(ticker):
    """Busca Trava de Baixa na cadeia, no DÉBITO (PUTs) ou no CRÉDITO (CALLs).

    Débito (PUTs): compra PUT de strike MAIOR + vende PUT de strike MENOR,
    mesmo vencimento. Ganha até o strike vendido (para baixo); risco
    limitado ao débito pago.

    Crédito (CALLs): vende CALL de strike MENOR + compra CALL de strike
    MAIOR (proteção), mesmo vencimento. Recebe o crédito de montagem; risco
    limitado à largura menos o crédito, acima do strike comprado.

    Espelho exato da Trava de Alta, com PUT/CALL trocados e os strikes
    invertidos (a comprada fica no strike MAIOR no débito, a vendida no
    strike MENOR no crédito — o oposto da trava de alta).

    Filtros:
      tipo       = 'debito' (PUTs) ou 'credito' (CALLs)
      exp        = índice do vencimento (0 = próximo mensal, 1 = +1, 2 = +2)
      tamanho_min/tamanho_max = largura da trava em R$ (distância entre
                   strikes), de 4 a 12
      risco_min/risco_max     = relação de risco, de 1 a 6:
                   ...
                   crédito (queremos baixo — trava mais conservadora)
      strike_pct = variação da perna de referência em % do spot (padrão 40 —
                   de 40% ITM a 40% OTM). No débito é a PUT comprada; no
                   crédito é a CALL vendida (a perna que fica perto do spot
                   e define a montagem)
      preco_min/preco_max = financeiro da montagem em R$, de -8 a 8
                   (negativo = crédito recebido, positivo = débito pago)
    """
    ...
    # Só o tipo de opção usado nesta variante (PUTs no débito, CALLs no crédito)
    is_call_leg = (tipo == 'credito')
    opts_by_exp = {}
    for o in opt_list:
        cat = str(o.get('category') or o.get('type') or '').upper()
        o_is_put = ('PUT' in cat or cat == 'P')
    ...
    def _pop_below(be, T_, iv):
        """P(S_T < be) em %, via log-normal risk-neutral — trava de baixa
        ganha ABAIXO do breakeven, ao contrário da trava de alta."""
        if be <= 0 or T_ <= 0 or not iv:
            return None
        d2 = (math.log(spot / be) + (r_cont - 0.5 * iv * iv) * T_) / (iv * math.sqrt(T_))
        return (1 - _norm_cdf(d2)) * 100

    # Faixa da perna de referência: comprada (PUT) no débito, vendida (CALL)
    # no crédito — ambas ficam perto do spot e são a perna que "escolhe" a
    # montagem (espelho exato da Trava de Alta).
    lo_k = spot * (1 - strike_pct / 100.0)
    hi_k = spot * (1 + strike_pct / 100.0)
    ref_cands = [o for o in opts if lo_k <= o['strike'] <= hi_k]
    ...
    for ref in ref_cands:
        ...
        if tipo == 'debito':
            ...
            # ref = CALL vendida (recebe o bid); busca comprada em strike
            # MAIOR (proteção acima, trava de baixa no crédito)
            ref_px = e_ref['bid_eff']
            if not ref_px or ref_px <= 0:
                continue
            outros = [o for o in opts if o['strike'] > ref['strike']]
        ...
        for outro in outros:
            ...
            if tipo == 'debito':
                ...
                if ratio < risco_min or ratio > risco_max:
                    continue
                be = ref['strike'] - custo        # ganha ABAIXO do breakeven
                buy_row, sell_row = ref, outro
                buy_px, sell_px = ref_px, out_px
                e_buy, e_sell = e_ref, e_out
            else:
                out_px = e_out['ask_eff']       # comprada (proteção): paga o ask
                if not out_px or out_px <= 0:
                    continue
                credito = round(ref_px - out_px, 2)        # >0 crédito esperado
                if credito <= 0.01:
                ...
                if ratio < risco_min or ratio > risco_max:
                    continue
                be = ref['strike'] + credito       # ganha ABAIXO do breakeven
                buy_row, sell_row = outro, ref
                buy_px, sell_px = out_px, ref_px
                e_buy, e_sell = e_out, e_ref

            iv = _iv_est(buy_row, T) or _iv_est(sell_row, T) or 0.35
            p_below = _pop_below(be, T, iv)
            pop = ev = ev_pct = None
            if p_below is not None:
                pop = p_below                     # trava de baixa: ganha abaixo do BE
                risco_ref = max_loss if max_loss else 0.001
                ev = pop / 100 * max_gain - (1 - pop / 100) * risco_ref
                ev_pct = round(ev / risco_ref * 100, 1)
    ...
    # Ordena pela relação crescente, como na Trava de Alta.
    rows.sort(key=lambda x: (x['ratio'], -x['liq']))
    rows = _fyt_diversify(rows, lambda x: x['buy_symbol'], per_key=3, limit=60)
